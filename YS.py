...
def random_dalvik_bd_working():
    """
    Generate a realistic Dalvik-style Facebook Android User-Agent tailored for BD users.
    Returns:
        str: A user agent string (Dalvik/2.1.0 ...)
    """
    fban = "FB4A"
    # Facebook app version pattern similar to the example
    facebook_version = f"{random.randint(350, 450)}.0.0.{random.randint(1, 40)}.{random.randint(50, 180)}"
    # realistic build version for the FB binary
    fbbv = str(random.randint(10000000, 99999999))
    density = random.choice(["1.0", "1.5", "2.0", "2.5", "3.0"])
    # common screen widths/heights for modern phones
    width = random.choice(["720", "1080", "1170", "1200", "1440"])
    height = random.choice(["1280", "1792", "1800", "1920", "2340", "2400", "2560"])
    fbcr = random.choice(["Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk", "Unknown"])
    fblc = random.choice(["en_US", "bn_BD"])
    fbpn = "com.facebook.katana"

    # Popular BD + global devices (updateable)
    devices = [
        ("Xiaomi", "Redmi Note 13"), ("Xiaomi", "Redmi Note 12"), ("Xiaomi", "Redmi Note 11 Pro"),
        ("Xiaomi", "Poco X6 Pro"), ("Xiaomi", "Poco F5"),
        ("Samsung", "Galaxy A15"), ("Samsung", "Galaxy A24"), ("Samsung", "Galaxy A54"),
    ...
    fbmf, fbdv = random.choice(devices)
    fbbd = fbmf  # brand as build device brand

    # Android versions commonly seen
    fbsv = random.choice(["10", "11", "12", "13", "14"])

    # Realistic build tags (examples — update if you want region-specific builds)
    build = random.choice([
        "UP1A.231005.007", "UP1A.240205.002", "TQ3A.230901.001",
        "TP1A.220624.014", "SP1A.210812.016", "RQ3A.210705.001",
        "RKQ1.200710.003", "QP1A.190711.020"
    ])
...
def random_dalvik_ua_all(seed: int | None = None) -> str:
    """
    Generate a realistic Dalvik-style Facebook Android User-Agent
    covering all popular BD + global Android devices.
    """
    if seed is not None:
        random.seed(seed)
    ...
    fbpn = "com.facebook.katana"
    fbca = "armeabi-v7a:armeabi"
    facebook_version = f"{random.randint(350, 450)}.0.0.{random.randint(1, 40)}.{random.randint(10, 180)}"
    fbbv = str(random.randint(10000000, 99999999))
    fbcr = random.choice(["Grameenphone", "Robi", "Banglalink", "Airtel", "Teletalk", "Unknown"])
    fblc = random.choice(["en_US", "bn_BD"])

    # Device database: (brand, model, default_android, density, width, height)
    devices = [
        ("Xiaomi", "Redmi Note 13", "13", "2.0", "1080", "2400"),
        ("Xiaomi", "Redmi Note 12", "12", "2.0", "1080", "2400"),
        ("Xiaomi", "Redmi Note 11 Pro", "11", "2.0", "1080", "2400"),
        ("Xiaomi", "Poco X6 Pro", "13", "2.5", "1220", "2712"),
        ("Xiaomi", "Poco F5", "13", "3.0", "1220", "2712"),
        ("Samsung", "Galaxy A15", "13", "2.0", "1080", "2340"),
        ("Samsung", "Galaxy A24", "13", "2.0", "1080", "2340"),
        ("Samsung", "Galaxy A54", "14", "3.0", "1080", "2340"),
        ("Samsung", "Galaxy S22 Ultra", "13", "3.5", "1440", "3088"),
        ("Samsung", "Galaxy S23", "14", "3.0", "1080", "2340"),
        ("Realme", "Realme C55", "13", "2.0", "1080", "2408"),
        ("Realme", "Realme Narzo 60", "13", "2.0", "1080", "2408"),
        ("Realme", "Realme 12 Pro", "14", "3.0", "1080", "2412"),
        ("Oppo", "Oppo A78", "13", "2.0", "1080", "2412"),
        ("Oppo", "Oppo F23", "13", "2.5", "1080", "2412"),
        ("Oppo", "Oppo Reno 10", "13", "3.0", "1080", "2412"),
        ("Vivo", "Vivo Y36", "13", "2.0", "1080", "2408"),
        ("Vivo", "Vivo V27", "13", "2.5", "1080", "2408"),
        ("Vivo", "Vivo V29e", "13", "3.0", "1080", "2412"),
        ("Infinix", "Infinix Note 30", "13", "2.0", "1080", "2460"),
        ("Infinix", "Infinix Hot 40i", "13", "2.0", "720", "1612"),
        ("Tecno", "Tecno Spark 20", "13", "2.0", "720", "1612"),
        ("Tecno", "Tecno Camon 20", "13", "2.0", "1080", "2408"),
        ("Symphony", "Symphony Z60", "12", "2.0", "720", "1600"),
        ("Walton", "Walton Primo H10", "11", "1.5", "720", "1600"),
        ("Itel", "Itel A70", "11", "1.5", "720", "1600"),
        ("Itel", "Itel S23", "12", "2.0", "720", "1600"),
    ]

    fbmf, fbdv, fbsv, density, width, height = random.choice(devices)

    # Android version randomization (device default or nearby version)
    fbsv = random.choice([fbsv, str(int(fbsv)-1), str(int(fbsv)), str(min(int(fbsv)+1, 14))])

    # Build tags mapped by Android version
    build_by_android = {
        "14": ["UP1A.240205.002", "UP1A.240401.001"],
        "13": ["UP1A.231005.007", "TP1A.230905.002", "TQ3A.230901.001"],
        "12": ["TP1A.220624.014", "SP1A.210812.016"],
        "11": ["RQ3A.210705.001", "RKQ1.200710.003"],
        "10": ["QP1A.190711.020", "RQ3A.200801.001"]
    }
    build = random.choice(build_by_android.get(fbsv, ["UP1A.231005.007"]))

    # Device metrics string
    fbdm = f"{{density={density},width={width},height={height}}}"

    END = (
        f"[FBAN/{fban};FBAV/{facebook_version};FBBV/{fbbv};"
        f"FBDM/{fbdm};FBLC/{fblc};FBCR/{fbcr};FBMF/{fbmf};FBBD/{fbmf};"
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 6.1
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
